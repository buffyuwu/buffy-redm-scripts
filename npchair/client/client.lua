    ...
    menu:addItem({
        ...
        onClick = function(currentData)
            if currentHairItem then
                SetMetaPedTag(playerPed, currentHairItem, `hair_gen_999_c0_999_ab`, `hair_gen_999_c0_999_nm`, `hair_gen_999_c0_999_m`, `metaped_tint_hair`, color1, color2, color3)
                UpdatePedVariation(playerPed)
				TriggerServerEvent('npchair:sync', GetPlayerServerId(PlayerId()), currentHairItem, color1, color2, color3)
            end
            jo.menu.show(false)
            inmenu = false
        end,
...
local function checkInitialized()
	...
	TriggerServerEvent('npchair:serversync', GetPlayerServerId(PlayerId()));
end
RegisterNetEvent('npchair:clientsync')
AddEventHandler('npchair:clientsync', function(hairData)
	isInit = true --the server is sending us a specific event, so we know the vorp check for a character passed in the server.lua file
	if not hairData[2] then return; end --the server didnt send us any hair data, so we will let the createthread below clobber it with requests once a second until we get data
	Wait(5000)
	TriggerEvent('chat:addMessage', {args = {"[Buffy Sync]", "Custom hair data found. Loading..."}, color = {100, 100, 100}})
	Wait(20000) --we got data! now to be safe lets wait for 20 seconds or so to make absolutely sure our player ped exists and is loaded in
	TriggerEvent('chat:addMessage', {args = {"[Buffy Sync]", "Custom hair loaded. If you need to reload it, use the /loadhair command."}, color = {100, 100, 100}})
	SetMetaPedTag(PlayerPedId(), hairData[1], `hair_gen_999_c0_999_ab`, `hair_gen_999_c0_999_nm`, `hair_gen_999_c0_999_m`, `metaped_tint_hair`, hairData[2], hairData[3], hairData[4])
...
Citizen.CreateThread(function()
	...
	while true do
		Citizen.Wait(1000)
		if not isInit then checkInitialized() end
	end
end)


RegisterCommand('loadhair',function()
	TriggerEvent('chat:addMessage', {args = {"[Buffy Sync]", "Manually loading custom hair, please wait up to 30 seconds..."}, color = {100, 100, 100}})
	TriggerServerEvent('npchair:serversync', GetPlayerServerId(PlayerId()))
end)
RegisterCommand('removenpchair',function()
	TriggerServerEvent('npchair:serversync', GetPlayerServerId(PlayerId()), true)
	ExecuteCommand('rc')
end)
